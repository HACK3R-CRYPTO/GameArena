...
contract ArenaPlatform is Ownable, ReentrancyGuard {
    ...
    enum MatchStatus { Proposed, Accepted, Completed, Cancelled }
    enum GameType { RockPaperScissors, DiceRoll, StrategyBattle, CoinFlip, TicTacToe }
    ...
    event MatchProposed(uint256 indexed matchId, address indexed challenger, address indexed opponent, uint256 wager, GameType gameType);
    ...
    function proposeMatch(address _opponent, GameType _gameType) external payable nonReentrant returns (uint256) {
        ...
        emit MatchProposed(matchId, msg.sender, _opponent, msg.value, _gameType);
        return matchId;
    }
    ...
    function acceptMatch(uint256 _matchId) external payable nonReentrant {
        ...
        require(m.opponent == address(0) || m.opponent == msg.sender, "Not your match");
    ...
    function playMove(uint256 _matchId, uint8 _move) external nonReentrant {
        ...
        require(msg.sender == m.challenger || msg.sender == m.opponent, "Not in match");
    ...
    function resolveMatch(uint256 _matchId, address _winner) external onlyOwner nonReentrant {
        Match storage m = matches[_matchId];
        require(m.status == MatchStatus.Accepted, "Match not in progress");
        require(_winner == m.challenger || _winner == m.opponent, "Invalid winner");
